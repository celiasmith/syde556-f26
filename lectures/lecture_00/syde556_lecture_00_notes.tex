...
\begin{itemize}
	\item \textbf{Course website}\\
		  ...
		  \begin{itemize}
		  	...
		  	\item \url{https://github.com/celiasmith/syde556-f26}
		  \end{itemize}
		  \emph{Note:} Any material on GitHub should be considered \enquote{preliminary} until officially linked from the course README.md. Until then, the material is still subject to change.
...
\begin{itemize}
	...
	\begin{itemize}
		\item The tests are worth 20 marks each and the lowest mark is dropped. (For SYDE 750 students, the final project is worth 20\% of the final mark and these are scaled to 80\%.)
		\item You have about two to three weeks for each practice notebook associated with the tests.
		\item You are free to discuss the notebooks with other students, but do not take any (written) notes during such discussions. Everyone must write their own code, generate their own graphs, and write their own answers (no AI).
		\item These notebooks (particularly the first two) are a lot of work, so start early.
	\end{itemize}
	\item \textbf{Final project} (worth 20\% for 750)
	\begin{itemize}
		\item (556 students if interested contact the instructor).
...
\begin{itemize}
	\item Get the textbook (\enquote{Neural Engineering}, Chris Eliasmith and Charles Anderson, 2003) (it's free on the website)
